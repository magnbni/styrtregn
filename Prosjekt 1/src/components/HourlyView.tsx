...
function HourlyView(props: { metData: Properties; day: string }) {
  ...
  const returnTableData = () => {
    return metData.timeseries
      .filter((e) => e.time.slice(0, 10) == day.slice(0, 10))
      .map((res) => (
        <>
          <tr key="time">
            <td>{res.time.slice(11, 16)}</td>
            <td style={{ width: "15%" }}>
              {res.data.next_1_hours?.summary.symbol_code ? (
                FetchIcon(res.data.next_1_hours?.summary.symbol_code)
              ) : (
                <p></p>
              )}
            </td>
            ...
            <td>
              {res.data.next_1_hours?.details.precipitation_amount
                ? res.data.next_1_hours.details.precipitation_amount
                : res.data.next_6_hours?.details.precipitation_amount}
            </td>
